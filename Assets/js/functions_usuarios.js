...
const App = {
    data() {
        ...
        getDatosIniciales:async function(){
            const response = await fetch(base_url+"/usuarios/getDatosIniciales");
        ...
        setFiltro:async function(tipo){
            if(tipo == "paises" && this.intPais != ""){
                ...
                const response = await fetch(base_url+"/usuarios/getEstados/estado/"+this.intPais);
                const objData = await response.json();
                this.arrDepartamentos = objData;
                this.arrCiudades = [];
            }else if(tipo == "departamentos" && this.intDepartamento != ""){
                const response = await fetch(base_url+"/usuarios/getEstados/ciudad/"+this.intDepartamento);
                const objData = await response.json();
                this.arrCiudades = objData;
            }
        },
        showModal:function(){
            this.strTituloModal = "Nuevo usuario";
        ...
        setDatos: async function(){
            ...
            const response = await fetch(base_url+"/usuarios/setUsuario",{method:"POST",body:formData});
            const objData = await response.json();
        ...
        getBuscar:async function (intPagina=1,strTipo = ""){
            ...
            const response = await fetch(base_url+"/usuarios/getBuscar",{method:"POST",body:formData});
            const objData = await response.json();
        ...
        getDatos:async function(intId){
          this.intId = intId;
          this.strTituloModal = "Editar usuario";
          const formData = new FormData();
          formData.append("id",this.intId);
          const response = await fetch(base_url+"/usuarios/getDatos",{method:"POST",body:formData});
          const objData = await response.json();
          if(objData.status){
        ...
        delDatos:function(intId){
            ...
            Swal.fire({
              ...
              if(result.isConfirmed){
                  ...
                  const response = await fetch(base_url+"/usuarios/delDatos",{method:"POST",body:formData});
                  const objData = await response.json();
                  if(objData.status){
